...
class stego:
    ...
    def main(self,win):    
        ...
        Button(win, text='Encode', width=25, font=('Times New Roman', 13), bg='SteelBlue', command=self.encode).place(
                x=240, y=100)
        Button(win, text='Decode', width=25, font=('Times New Roman', 13), bg='SteelBlue', command=self.decode).place(
                x=240, y=160)
        win.update()
    ...
    def encode(self):
        ...
        if img_path.get() != None and text_to_be_encoded.get() != None and after_save_path.get() != None:
            ...
            self.main_encryption(img_path.get(), text_to_be_encoded.get(), after_save_path.get(), pwd.get())).place(x=220, y=215)
        else:
    ...
    def select_file(self):
        gloimg_path= filedialog.askopenfilename(title="Select an Image", filetype=(('image    files','*.jpg'),('all files','*.*')))
       
            
            
    def main_encryption(self,img, message, new_image_name, pwd):
        ...
        text = self.encrypt(message, pwd)
        new_image = image.copy()
        self.encryption(new_image, text)
        new_image_name += '.png'
    ...
    def encryption(self,img, data):
        # This method will encode data to the new image that will be created
        si = img.size[0]
        (x, y) = (0, 0)

        for pixel in self.generate_data(img.getdata(), data):
    ...
    def generate_data(self,pixels, data):
        """This function takes two arguments
    ...
    def decode(self):
        ...
        self.main_decryption(img_name.get(), text_strvar, de_pass.get())).place(x=220, y=140)
    ...
    def main_decryption(self,img, strvar, pwd):
        ...
        data = ''
        self.image_data = iter(image.getdata())

        while True:
            try:
                pixels = [value for value in self.image_data.__next__()[:3] + self.image_data.__next__()[:3] + self.image_data.__next__()[:3]]
            except:
                ...
                if pixels[-1] % 2 != 0:
                    strvar.set(self.decrypt(data, pwd))
                    break

    def decrypt(self,text, pwd):
        '''
        This function takes two arguments
        ...
        ii. If does not mathes gives new wrong message everytime
